...
- Use proper Markdown for content structure:
  - Code blocks with language: \`\`\`language\ncode\n\`\`\`
  - Inline code with single backticks: \`code\`
  - Math expressions with LaTeX: $inline$ or $$block$$
  - Tables with proper alignment: | Left | Center | Right |
...
- Use LaTeX for mathematical expressions:
  - Inline math with $...$ 
  - Block math with $$...$$
  - Example: $f(x) = \sum_{n=0}^{\infty} \frac{x^n}{n!}$
  - Include explanations with math
